...
- `backend/services/stockScanner.js` — 682-stock scanner, auto-runs every 2h via setInterval
...
3. **Scanner** (`backend/services/stockScanner.js`) — runs across all 682 stocks (confirmed exact count) every 2h, caches results in `ScanResult` (key:'latest'). CONFIRMED by reading the code (not guessed): it uses `getEngineRecommendation`/`getNewsSentiment` from `yahooFinance.js` — i.e. the SAME technical-indicator logic as the Free Signal Engine (Yahoo-only), plus Finnhub keyword-based news scoring (POSITIVE/NEGATIVE word lists + analyst recommendation split) — NOT Pro Engine, NOT Claude AI news analysis. Two-phase: Phase 1 scans all 682 technically (Yahoo only); Phase 2 enriches only the top 120 technical candidates with Finnhub news/analyst data (throttled — batches of 2 symbols / 8s ≈ 30 req/min, well under Finnhub's 60/min shared-with-production quota), then re-sorts by combined score. Previously Phase 1 silently called Finnhub for ALL 682 stocks too (a `getEngineRecommendation(symbol, {skipNews:true})` bug — the function didn't accept a second arg at all, so `skipNews` did nothing) — this was the root cause of the "Too many requests" failures; fixed by giving `getEngineRecommendation` a real `opts.skipNews` and wiring Phase 2 (previously dead code) back in with throttling.
