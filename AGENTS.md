...
- Automated status: backend/frontend syntax and inline HTML scripts are clean, `npm test` passes 43/43, and the local Mongo sentiment service was validated read-only against live local records. A real Yahoo-only NVDA check verified the exact 2026-08-18 OHLC/daily move and 2026-08-10→2026-08-18 period return.
- Design comfort pass completed locally: the dark identity now uses clearer layered surfaces and stronger secondary-text contrast; the home hero is less repetitive; its claim accurately says “2,000-stock universe”; failed live quotes collapse the ticker instead of leaving an empty strip; anchored Engine navigation clears the sticky header; the Engine card no longer stretches to the registration form; and the mobile chat launcher is smaller and no longer pulses. Desktop and mobile English flows were inspected in Safari; Arabic/Hebrew copy and direction-dependent layout were checked statically.
- Chat relevance now distinguishes watchlist research interests from owned positions and keeps unrelated rejected Scanner/watchlist candidates out of broad personalized recommendations. The selection-to-reply popup accepts short words and long answers, uses correct fixed viewport coordinates on scrolled pages, and explicitly enables Safari text selection.
