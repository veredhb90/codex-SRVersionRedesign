...
const Onboarding = {
  ...
  show() {
    ...
    modal.style.cssText = 'position:fixed;inset:0;z-index:20000;background:rgba(8,15,36,0.85);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;';
    modal.innerHTML = `
      <div style="background:#fff;border-radius:24px;padding:0;max-width:520px;width:100%;box-shadow:0 32px 80px rgba(0,0,0,0.3);overflow:hidden;animation:obIn .4s cubic-bezier(.34,1.56,.64,1);">
        <style>@keyframes obIn{from{opacity:0;transform:scale(.9) translateY(20px)}to{opacity:1;transform:scale(1) translateY(0)}}</style>
        
        <!-- Header -->
        <div style="background:linear-gradient(135deg,#0D47A1,#1565C0,#1976D2);padding:28px 32px;color:#fff;text-align:center;">
          <div style="font-size:40px;margin-bottom:8px;">🎯</div>
          <button onclick="Onboarding.skip()" style="position:absolute;top:16px;right:16px;background:rgba(255,255,255,0.2);border:none;color:#fff;width:30px;height:30px;border-radius:50%;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;">✕</button><h2 style="font-size:22px;font-weight:800;margin:0 0 8px;letter-spacing:-0.5px;">Your Trader Profile</h2>
  ...
  nextStep(step) {
    ...
    if (step === 2) {
      ...
      if (!age || age < 18 || age > 100) { alert('Please enter your age (18-100)'); return; }
      if (!Onboarding.selections.amount) { alert('Please select your investment amount'); return; }
    }
    if (step === 3) {
      if (!Onboarding.selections.style) { alert('Please select your trading style'); return; }
      if (!Onboarding.selections.experience) { alert('Please select your experience level'); return; }
    }
  ...
  async save() {
    if (!Onboarding.selections.risk) { alert('Please select your risk tolerance'); return; }
