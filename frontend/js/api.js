const API = {
  ...
  popular:             (all)     => API._req('/recommendations/popular' + (all ? '?all=true' : '')),
  ...
  postComment:         (id,text,parentCommentId) => API._req('/recommendations/'+id+'/comment',{ method:'POST', body:JSON.stringify({ text, parentCommentId:parentCommentId || undefined }) }),
  ...
  myPaymentHistory:    ()        => API._req('/payments/my-history'),
  cancelSubscription:  ()        => API._req('/payments/cancel', { method:'POST' }),
  ...
  scanTop5:            () => API._req('/scanner/top5'),
