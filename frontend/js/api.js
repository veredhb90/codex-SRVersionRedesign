...
const API = {
  ...
  // Recommendations
  feedAll:       (p=1) => API._req('/recommendations/feed?page='+p),
  ...
  repost:        (id, comment) => API._req('/recommendations/'+id+'/repost', { method:'POST', body:JSON.stringify({ comment: comment||'' }) }),
