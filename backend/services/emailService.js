...
`;
const logo = `<h1 style="color:#00e676;margin:0 0 4px;font-size:28px;letter-spacing:2px;">SWING<span style="color:#ff1744;">RUSH</span></h1>
<p style="color:#555;margin:0 0 32px;font-size:13px;">Financial Recommendation Network</p>`;
...
const sendPassword = async (to, password) => {
  await transporter.sendMail({
    ...
    html: `<div style="${baseStyle}">${logo}
      ...
      <p style="color:#555;margin-top:24px;font-size:13px;">Login at <a href="${process.env.CLIENT_URL}/login.html" style="color:#00e676;">SwingRush</a>. You can change your password after login.</p>
    </div>`,
  });
};
...
const sendFollowAlert = async (to, followerName, recSymbol, direction, takeProfit) => {
  ...
  await transporter.sendMail({
    ...
    html: `<div style="${baseStyle}">${logo}
      <p style="font-size:16px;margin-bottom:20px;">Someone you follow just shared a new recommendation:</p>
      <div style="background:#111;border:2px solid ${color};border-radius:12px;padding:20px;">
        ...
        <div style="color:#aaa;font-size:13px;margin-top:4px;">Posted by <strong style="color:#fff;">${followerName}</strong></div>
      </div>
...
module.exports = { sendOTP, sendPassword, sendFollowAlert };
